...
class App(ctk.CTk):
    ...
    def __init__(self, db_connection, chat_service, inventory_service, recipe_service, shopping_list_service):
        super().__init__()
        
        # Armazena a conexão e todos os serviços para poder passá-los para outras janelas
    ...
    def _abrir_tela(self, TelaClasse, **kwargs):
        """
        Função genérica para abrir uma nova tela, escondendo o menu.
        """
        self.withdraw()  # Esconde a janela do menu
        
        # Cria a nova janela (ex: ChatApp(chat_service=...))
        # O callback para reabrir o menu é passado aqui
        nova_tela = TelaClasse(master=self, on_close_callback=lambda: self._reabrir_menu(nova_tela), **kwargs)
    ...
    def _criar_widgets(self):
        ...
        # O caminho para a pasta 'assets' agora parte da raiz do projeto
        assets_path = Path(__file__).parent.parent / "assets"
        
        user_icon_image = ctk.CTkImage(Image.open(assets_path / "frame1" / "user_icon.png").resize((32, 32), Image.LANCZOS), size=(40, 40))
        ctk.CTkButton(header_frame, text="", image=user_icon_image, width=45, height=45, fg_color="transparent", hover_color=self.BUTTON_HOVER_COLOR).pack(side="left", padx=10, pady=10)

        options_image = ctk.CTkImage(Image.open(assets_path / "frame1" / "options.png").resize((32, 32), Image.LANCZOS), size=(30, 30))
        ctk.CTkButton(header_frame, text="", image=options_image, width=40, height=40, fg_color="transparent", hover_color=self.BUTTON_HOVER_COLOR).pack(side="right", padx=5, pady=5)
        ...
        target_width = 280
        aspect_ratio = original_logo_image.height / original_logo_image.width
        ...
        # --- MUDANÇA CRUCIAL NOS COMANDOS DOS BOTÕES ---
        ...
        ctk.CTkButton(buttons_frame, text="LISTA DE COMPRAS", command=lambda: self._abrir_tela(ShoppingListApp, shopping_list_service=self.shopping_list_service), height=55, font=self.button_font, fg_color=self.BUTTON_COLOR, hover_color=self.BUTTON_HOVER_COLOR).grid(row=3, column=0, padx=20, pady=(10, 20), sticky="ew")
